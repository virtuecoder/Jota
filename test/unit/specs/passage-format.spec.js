...
test('Should format single verse', () => {
  verifyFormat({
    passages: [{'Gen' : {'1' : {'3' : 'And God said, Let there be light: and there was light.'}}}],
...
test('Should format multiple passages', () => {
  verifyFormat({
    passages: [
...
function verifyFormat({passages, template, expected}) {
  ...
  expect(formatter.format(passages)).toBe(expected)
}
